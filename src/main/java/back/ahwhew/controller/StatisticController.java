...
public class StatisticController {
    ...
    public ResponseEntity <String> getMeme(@RequestParam(required = false) Character gender,
                          @RequestParam(required = false) String age){
        try{
            LocalDate endDate = LocalDate.now();
            LocalDate startDate = endDate.minusMonths(1); //한달 전
            ...
            if (gender != null && age != null) {
                ...
                // 카테고리 선택 안했으면 전체 유저 평균 데이터 전송
                gifUrl = statisticsService.getMemeByUser(startDate, endDate);
            }

            return ResponseEntity.ok().body(gifUrl);
        }catch (Exception e) {
            log.error("에러 발생: {}", e.getMessage(), e);
            // 예외에 대한 적절한 응답을 클라이언트에게 반환하거나, 기본값이나 빈 결과를 반환하는 등의 처리를 수행할 수 있습니다.
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
